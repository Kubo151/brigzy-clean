    ...
    return (
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 15 }}>
            <View style={{ flex: 1, marginRight: 12 }}>
                <Text style={{ fontSize: 15.5, fontWeight: '700', color: C.text, letterSpacing: -0.2 }}>{label}</Text>
                <Text style={{ fontSize: 12.5, color: C.muted, marginTop: 3, fontWeight: '500' }}>{subtitle}</Text>
            </View>
            <Switch
                value={value}
                onValueChange={onToggle}
                trackColor={{ false: C.cLo, true: C.accent }}
                thumbColor="#FFF"
                ios_backgroundColor={C.cLo}
...
export default function NotificationsScreen() {
    ...
    const Divider = () => <View style={{ height: 1, backgroundColor: C.hair, marginLeft: 16 }} />;
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={['top']}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40, paddingTop: 8 }}>
                ...
                <Section icon={<Mail size={16} color={C.star} strokeWidth={2} />} iconBg="rgba(255,179,0,0.15)" label="EMAIL NOTIFIKÁCIE">
                ...
                <Section icon={<Smartphone size={16} color={C.verified} strokeWidth={2} />} iconBg="rgba(46,168,255,0.15)" label="V APLIKÁCII">
