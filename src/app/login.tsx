...
function InputField({ icon: Icon, placeholder, value, onChangeText, secure, keyboard, editable = true, C, loading }: {
    ...
    secure?: boolean; keyboard?: string; editable?: boolean; C: FlintColors; loading: boolean;
}) {
    const [showPassword, setShowPassword] = useState(false);
    return (
        <View style={[styles.inputRow, { backgroundColor: C.card2 }]}>
...
// ── Brand logo ──
function BrandLogo({ C }: { C: FlintColors }) {
    return (
...
function SubmitButton({ label, onPress, loading, C }: {
    label: string; onPress: () => void; loading: boolean; C: FlintColors;
}) {
    return (
        <Pressable
            onPress={onPress}
            ...
            style={({ pressed }) => [
                styles.submitBtn,
                { backgroundColor: C.accent, opacity: loading ? 0.6 : pressed ? 0.9 : 1 },
...
function SocialButton({ label, iconType, C, onPress }: {
    label: string; iconType: 'apple' | 'google'; C: FlintColors; onPress: () => void;
}) {
    return (
        <Pressable onPress={onPress} style={({ pressed }) => [styles.socialBtn, { backgroundColor: C.card }, pressed && { opacity: 0.85, transform: [{ scale: 0.99 }] }]}>
            {iconType === 'apple' ? (
...
export default function Login() {
    ...
    const countryTheme = {
        backgroundColor: C.card2, onBackgroundTextColor: C.text, fontSize: 15,
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={[styles.flex, { backgroundColor: C.bg }]}>
                <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
                    ...
                    <View style={styles.formGroup}>
                        ...
                        <View style={[styles.phoneRow, { backgroundColor: C.card2 }]}>
                            <Pressable onPress={() => setCountryPickerVisible(true)} style={[styles.countryBtn, { borderRightColor: C.divider }]}>
...
const styles = StyleSheet.create({
    ...
    logo: { width: 68, height: 68, borderRadius: RADIUS.lg, alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
    brandName: { fontSize: 40, fontWeight: '800', letterSpacing: -0.5, marginBottom: 6 },
    brandSub: { fontSize: 16, fontWeight: '600' },

    formGroup: { gap: 14 },
    inputRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 14, gap: 10, borderRadius: RADIUS.md },
    input: { flex: 1, fontSize: 16, fontWeight: '500' },

    phoneLabel: { fontSize: 13, fontWeight: '700', marginBottom: -6, marginLeft: 4 },
    phoneRow: { flexDirection: 'row', alignItems: 'center', borderRadius: RADIUS.md },
    ...
    socialBtn: { height: 54, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 12, borderRadius: RADIUS.md },
